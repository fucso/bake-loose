...
本規約は `backend/tests/schema_naming.rs` の統合テストで機械的に検証する。
`#[sqlx::test]` が生成する使い捨てデータベースに全マイグレーションを適用し、
`pg_constraint` / `pg_index` を走査して主キー・一意制約・外部キー・インデックスの名前が
上表のとおりかを検査する。違反があればテストが fail する。

マイグレーションを追加したら、実 DB へ `sqlx migrate run` する**前に** `cargo test` を実行すること
（手順は `AGENTS.md` 4 章「マイグレーション作成から適用までの手順」を参照）。
