# Spec: Trial 詳細（記録ワークベンチ）

> Feature Issue: #83, #84

## 概要

1 つの Trial（試行）に紐づく Step（工程）と Parameter（記録要素）を時系列で俯瞰し、
Trial 自体の編集・完了と、工程の記録（追加・編集・完了）を行う画面。Trial 機能の中心となる画面であり、
調理中の記録操作はこの画面の上で完結させることを前提に情報設計している。

## 要件

### ルーティング

| パス | 画面 |
|------|------|
| `/projects/:id/trials/:trialId` | Trial 詳細（記録ワークベンチ） |

`:id` はプロジェクト ID、`:trialId` は Trial ID。「← 戻る」で `/projects/:id` に戻る。

### 表示内容

- ヘッダ: Trial の `name`・`memo`・`status` バッジ・`completedAt`（完了済みの場合）
  - `name` は任意項目のため、未設定時は「名称未設定の試行」を表示する
- タイムライン: Step を `position` 昇順に並べたカード列。各カードに配下の Parameter を表示する

### 操作

| 操作 | 使用する GraphQL |
|------|-----------------|
| Trial の name / memo の編集 | `updateTrial(id, input: UpdateTrialInput)` |
| Trial の完了 | `completeTrial(id)` |
| 工程の追加 | `addStep(trialId, input: AddStepInput)` |
| 工程の name / startedAt の編集 | `updateStep(trialId, stepId, input: UpdateStepInput)` |
| 工程の完了 | `completeStep(trialId, stepId)` |

Parameter の記録操作（追加・編集・削除）は本画面のスコープ外で、
このワークベンチの上に別途実装する。本仕様の時点では Parameter は読み取り専用。

### 状態表示

| 状態 | 表示 |
|------|------|
| 取得中 | `LoadingSpinner` |
| 取得失敗 | `ErrorState`（再試行ボタンあり） |
| `trial(id)` が null / `trialId` 不在 | `ErrorState`「指定された試行が見つかりません」 |
| Step が 0 件 | `EmptyState`「まだ工程が記録されていません」 |

## アーキテクチャ設計・実装方針

### 情報設計: 1 画面に集約される情報のコントロール

この画面には「複数 Step × 各 Step 複数 Parameter」が一度に載る。
モバイル（PWA）で調理中に片手操作されることを前提に、以下の方針で情報量を制御する。

#### 1. フォーカス外の情報を畳む

- Step カードは折りたたみ可能とし、**未完了の工程は展開・完了済みの工程は畳んだ状態**を初期表示とする。
  記録の焦点は「これから記録する工程」にあり、完了済みの工程は参照頻度が下がるため。
- Trial が完了済み（`status: COMPLETED`）の場合は焦点となる工程が存在しないため、
  **全工程を畳んだ俯瞰表示**を初期状態とする。
- 畳んだ状態でもヘッダに「工程名 / 状態 / パラメーター件数 / 開始・完了日時」のサマリーを常に出し、
  展開しなくても進捗を追えるようにする。
- 開閉はユーザーが手動で切り替えられる。**手動操作は初期表示の方針より優先**し、
  再取得でデータが更新されても開閉状態は維持する。
  そのためローディング表示は初回取得時のみとし、更新後の再取得では内容を表示したままにする。
  再取得のたびに内容を差し替えると、工程カードの開閉状態とスクロール位置が失われ記録操作を妨げるため。

#### 2. 現在記録中の工程へのフォーカス誘導

- `position` 昇順で**最初の未完了 Step** を「記録中の工程」とみなす。
- 記録中の工程はカード枠線を強調し、`aria-current="step"` を付与して支援技術にも伝える。
- 初期表示時および記録中の工程が切り替わったとき、その工程を画面内へスクロールする。
  工程数が増えても、画面を開いた直後に記録対象が視界に入る状態を保つ。

#### 3. モバイルでのスクロール量と可読性

- Parameter は種別によらず「ラベル + 値」の 1 行に正規化して表示し、カード内の縦方向の伸びを抑える。
- Trial のステータス（記録中 / 完了）と工程の状態（未着手 / 進行中 / 完了）は語彙を分け、
  同じ画面に並んだときにどちらの状態を指すか一目で区別できるようにする。

この情報設計は、後続で実装する Step 記録・Parameter 記録の UI 配置の前提になる。
新しい記録操作を追加する場合も「記録中の工程が常に展開・可視である」状態を崩さないこと。

### 工程の記録操作

#### 操作の配置

- 工程の追加はタイムラインの末尾に置く。追加された工程は `position` の末尾に積まれるため、
  操作の位置と結果の位置を一致させる。工程が 0 件（空状態）のときも追加操作は表示する。
- 工程の編集・完了は工程カードの本文（折りたたみ対象の領域）内に置く。
  記録中の工程は常に展開されるため、画面を開いた直後から記録対象の操作が視界に入る。
  カードのヘッダは開閉のトグルであり、その中に操作を入れ子にはしない。
- 完了済みの工程・完了済み Trial では編集・完了・追加を行えない
  （バックエンドの不変条件と一致させ、実行できない操作は表示しない）。

#### 入力

追加と編集は入力項目（`name` 必須・`startedAt` 任意）が同一のため、
入力モーダルを 1 つに集約し、送信先のミューテーションだけを呼び出し元で切り替える。

| 項目 | 必須 | 入力方法 |
|------|------|----------|
| 工程名 | 必須 | テキスト入力。空欄・空白のみは送信前に弾く |
| 開始日時 | 任意 | `<input type="datetime-local">`。空欄は未設定 |

`updateStep` の `startedAt` は未指定（undefined）で変更なし、`null` でクリアされる。
編集フォームは常に現在値を表示し `name` / `startedAt` の両方を送るため、
**開始日時の空欄は「未設定に戻す」意図として `null` を送る**（Trial の name / memo と同じ方針）。

`completeStep` の `completedAt` は送らない。完了操作は「今この工程を終えた」という意味であり、
バックエンドが現在時刻を設定する（`completeTrial` と同じ方針）。

#### 結果の反映

記録操作の成功後は Trial 詳細のクエリを再取得し、タイムライン全体に反映する。
工程の position・完了状態・記録中の工程の判定はいずれもサーバー側の状態から導出されるため、
個々の操作結果を画面側で継ぎ足さず、再取得した一貫した状態で描画する。
再取得中も内容は表示し続けるため、工程カードの開閉状態とスクロール位置は維持される。

エラーは操作ごとに発生源の近くへ出す。追加・編集の失敗は入力モーダル内に表示して
入力内容を保ったまま再試行でき、完了の失敗は工程カード内の操作の近くに表示する。

失敗メッセージは次の操作を始めた時点で破棄する。入力モーダルを開き直したとき・完了を再試行したときに
前回の失敗が残っていると、まだ何もしていない操作が失敗したように見えるため。
また送信中は Esc キー・背景クリックによる閉じる操作を受け付けない。送信中に閉じると結果の表示先が
無くなり、失敗がユーザーに伝わらないため。編集と完了は一方の送信中にもう一方を実行できない
（2 つの更新が並走すると、どちらの結果が最後に反映されるかが不定になるため）。

### データ取得

Step と Parameter は 1 画面で俯瞰するため、`trial(id)` クエリで
`steps { ... parameters { id parameterType content } }` まで一度に取得する。
工程ごとの追加リクエストは行わない。

「← 戻る」の遷移先には、取得した Trial の `projectId` を使う（未取得時のみ URL の `:id` にフォールバックする）。
URL の `:id` は利用者が書き換えられるため、実際の所属プロジェクトと食い違っていても正しい一覧へ戻れるようにする。

Parameter の種別判定には、バックエンドが公開する `parameterType`
（`KEY_VALUE` / `DURATION` / `TIME_MARKER` / `TEXT`）を使う。
これを TypeScript の判別子付き合併型のタグにすることで、JSON スカラーである
`content` の構造が種別ごとに型で絞り込まれる。

### Parameter の表示整形

各種別を「ラベル + 値」に正規化する。

| 種別 | ラベル | 値の例 |
|------|--------|--------|
| `KEY_VALUE` | `key` | `300g`（quantity） / `冷蔵庫`（text） |
| `DURATION` | `note`（空なら「経過時間」） | `90分` |
| `TIME_MARKER` | `note`（空なら「時間マーカー」） | `30分時点` |
| `TEXT` | なし | 記述内容そのまま |

時間量の単位は `day` / `hour` / `minute` / `second` を `日` / `時間` / `分` / `秒` に変換する。

`content` は JSON スカラーとして届くため、バックエンドに種別が追加されるとフロントの型より先に
未知の値が流れてくる。その場合は画面全体を落とさず、`content` をそのまま文字列化して表示する。

### 日時の表示と入力

バックエンドは日時を JST オフセット付き ISO 8601 で返すため、
閲覧端末のタイムゾーン設定に関わらず JST（`Asia/Tokyo`）で `2026/01/01 09:00` 形式に整形する。

`<input type="datetime-local">` はタイムゾーンを持たない「壁時計の時刻」を扱うため、
表示と同じく JST の壁時計時刻に変換して流し込み、送信時は JST オフセット（`+09:00`）を付けて
ISO 8601 文字列に戻す。閲覧端末のタイムゾーンで記録時刻がずれることを防ぐ。

### name / memo の更新セマンティクス

`UpdateTrialInput` の `name` / `memo` は未指定なら変更なし、`null` ならクリアされる。
編集フォームは両方を常に送信し、**空欄は「未設定に戻す」意図として `null` を送る**。

## エンドポイント仕様

本画面は既存の GraphQL API のみを利用し、バックエンドの変更はない。

```graphql
query Trial($id: ID!) {
  trial(id: $id) {
    id
    projectId
    name
    memo
    status        # IN_PROGRESS | COMPLETED
    completedAt
    steps {
      id
      name
      position
      startedAt
      completedAt
      isCompleted
      parameters {
        id
        parameterType   # KEY_VALUE | DURATION | TIME_MARKER | TEXT
        content         # JSON スカラー
      }
    }
  }
}

mutation UpdateTrial($id: ID!, $input: UpdateTrialInput!) {
  updateTrial(id: $id, input: $input) { id name memo }
}

# completedAt を省略するとバックエンドが現在時刻を設定する
mutation CompleteTrial($id: ID!) {
  completeTrial(id: $id) { id status completedAt }
}

# position は既存の工程数から自動採番されるため入力に含めない
mutation AddStep($trialId: ID!, $input: AddStepInput!) {
  addStep(trialId: $trialId, input: $input) { id name position startedAt }
}

# startedAt は未指定で変更なし、null でクリア（MaybeUndefined）
mutation UpdateStep($trialId: ID!, $stepId: ID!, $input: UpdateStepInput!) {
  updateStep(trialId: $trialId, stepId: $stepId, input: $input) { id name startedAt }
}

# completedAt を省略するとバックエンドが現在時刻を設定する
mutation CompleteStep($trialId: ID!, $stepId: ID!) {
  completeStep(trialId: $trialId, stepId: $stepId) { id isCompleted completedAt }
}
```

工程名はバックエンドで「空文字・空白のみは不可、100 文字以内」が検証される。
また `addStep` / `updateStep` / `completeStep` はいずれも記録中の Trial に対してのみ実行でき、
`updateStep` / `completeStep` は未完了の工程に対してのみ実行できる。

`content` の JSON 構造は `backend/src/domain/models/parameter.rs` の定義に従う。

## 意思決定記録

| 決定事項 | 採用した方針 | 理由 | 関連 Issue |
|---------|------------|------|-----------|
| Step の初期開閉状態 | 未完了は展開・完了済みは畳む。完了済み Trial は全て畳む | 記録の焦点である未完了工程を常に可視にしつつ、モバイルでのスクロール量を抑えるため | #83 |
| 開閉状態の同期 | 手動操作を初期方針より優先し、再取得でも維持する | 記録中にデータを再取得するたび開閉が戻ると操作を妨げるため | #83 |
| 記録中の工程の定義 | `position` 昇順で最初の未完了 Step | Step 自体は「現在の工程」を持たないため、順序と完了状態から一意に導出する | #83 |
| Parameter の種別判定 | `content` の JSON タグではなく `parameterType` を使う | バックエンドが JSON を解析せず種別判定できるよう公開している列挙子であり、TypeScript の型の絞り込みにも使えるため | #83 |
| Step / Parameter の取得 | `trial(id)` で一括取得 | 1 画面で全工程を俯瞰する画面であり、工程ごとの追加リクエストは無駄になるため | #83 |
| 日時の表示タイムゾーン | 常に JST に固定 | バックエンドが JST を前提に日時を返しており、閲覧端末のロケールで記録時刻がずれて見えることを防ぐため | #83 |
| 空欄の name / memo | `null` を送りクリアする | いずれも任意項目であり、空欄にした意図は「未設定に戻す」であるため | #83 |
| 再取得中の表示 | ローディング表示は初回取得時のみ。更新後の再取得では内容を表示したままにする | 内容を差し替えると工程カードの開閉状態とスクロール位置が失われ、記録操作を妨げるため | #83 |
| 「← 戻る」の遷移先 | 取得した Trial の `projectId` を使い、未取得時のみ URL の `:id` にフォールバックする | URL の `:id` が実際の所属プロジェクトと食い違っていても正しい一覧へ戻せるため | #83 |
| 未知の Parameter 種別 | 画面を落とさず `content` をそのまま文字列化して表示する | `content` は JSON スカラーのため、バックエンドの種別追加がフロントの型より先に届きうるため | #83 |
| Trial と Step の状態ラベル | 語彙を分ける（Trial: 記録中 / 完了、Step: 未着手 / 進行中 / 完了） | 同じ画面に両方の状態が並ぶため、どちらを指すか区別できるようにする | #83 |
| 工程の追加・編集の入力 UI | 入力モーダル（`components/ui/dialog.tsx`）に集約し、追加と編集で共用する | 入力項目が同一で送信先だけが異なるため。既存の唯一の入力 UI（`CreateProjectModal`）と操作感を揃え、dialog のバリアントを増やさない | #84 |
| 工程の編集・完了操作の配置 | 工程カードの本文内（折りたたみ対象の領域） | 記録中の工程は常に展開されるため操作は可視のまま保たれ、開閉トグルであるカードヘッダにボタンを入れ子にせずに済むため | #84 |
| 工程の追加操作の配置 | タイムラインの末尾（工程 0 件のときも表示する） | 追加された工程は position の末尾に積まれるため、操作の位置と結果の位置を一致させる。空状態からも記録を始められるようにする | #84 |
| 記録操作を出す条件 | 記録中の Trial かつ未完了の工程のみ | バックエンドが完了済みの Trial・工程への変更を拒否するため、実行できない操作は表示しない | #84 |
| 工程の開始日時の空欄 | `null` を送りクリアする | 任意項目であり、フォームは常に現在値を表示するため、空欄にした意図は「未設定に戻す」であるため（Trial の name / memo と同じ） | #84 |
| 工程完了時の completedAt | 送らずバックエンドの現在時刻に委ねる | 完了操作は「今この工程を終えた」という意味であり、`completeTrial` と方針を揃える | #84 |
| 記録操作後の画面更新 | 個々の操作結果を継ぎ足さず Trial 詳細を再取得する | position・完了状態・記録中の工程はいずれもサーバー側の状態から導出されるため、一貫した状態で描画する方が破綻しない | #84 |
| 記録操作の UI 上の語彙 | 「工程」に統一する（Issue 本文の「ステップ」は用いない） | 画面内の既存表記が「工程」であり、Trial と Step の状態ラベルを区別する方針とも整合するため | #84 |
| エラーの表示位置 | 追加・編集は入力モーダル内、完了は工程カード内の操作の近く | 失敗した操作の発生源で気づけるようにし、入力内容を保ったまま再試行できるようにするため | #84 |
| 失敗メッセージの保持期間 | 次の操作を始めた時点で破棄する（ミューテーションの状態をそのまま表示しない） | 開き直した入力モーダルに前回の失敗が残ると、まだ何もしていない操作が失敗したように見えるため | #84 |
| 送信中の閉じる操作 | Esc・背景クリックを受け付けない | 送信中に閉じると結果の表示先が無くなり、失敗がユーザーに伝わらないため | #84 |
| 編集と完了の同時実行 | 一方の送信中は両方の操作を無効化する | 2 つの更新が並走すると、どちらの結果が最後に反映されるかが不定になるため | #84 |
