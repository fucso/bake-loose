...
/// 外部キーのカラム名の接尾辞
///
/// 外部キーのカラム名は `{参照先テーブルの単数形}_id` とするため、
/// この接尾辞を取り除くと参照先のエンティティ名が得られる。
pub(crate) const FOREIGN_KEY_COLUMN_SUFFIX: &str = "_id";
...
/// インデックス名の接頭辞（`idx_`）を取り除く
///
/// 一致しない場合は元の文字列をそのまま返す。
pub(crate) fn strip_index_prefix(name: &str) -> &str {
    name.strip_prefix(INDEX_PREFIX).unwrap_or(name)
}

/// 一意制約名の接尾辞（`_key`）を取り除く
///
/// 一致しない場合は元の文字列をそのまま返す。
pub(crate) fn strip_unique_suffix(name: &str) -> &str {
    name.strip_suffix(UNIQUE_CONSTRAINT_SUFFIX).unwrap_or(name)
}
...
mod tests {
    ...
    #[test]
    fn test_strip_index_prefix_removes_index_prefix() {
        assert_eq!(strip_index_prefix("idx_projects_name"), "projects_name");
        assert_eq!(strip_index_prefix("projects_pkey"), "projects_pkey");
    }

    #[test]
    fn test_strip_unique_suffix_removes_unique_suffix() {
        assert_eq!(
            strip_unique_suffix("steps_trial_id_position_key"),
            "steps_trial_id_position"
        );
        assert_eq!(
            strip_unique_suffix("idx_projects_name"),
            "idx_projects_name"
        );
    }
    ...
    fn test_real_constraint_names_follow_conventions() {
        ...
        for (name, referencing, referenced_column) in [
            ...
            assert!(referenced_column.ends_with(FOREIGN_KEY_COLUMN_SUFFIX));
        }
    }
}
